...
pub struct CreateFrameRequest {
    ...
    /// frame. Existing users are NOT backfilled by toggling this
...
pub async fn create_frame(
    ...
    Json(payload): Json<CreateFrameRequest>,
) -> Result<(StatusCode, Json<FrameRow>), StatusCode> {
    ...
    let row = sqlx::query_as::<_, FrameRow>(
        r#"
        INSERT INTO frames (name, price, currency, is_default)
        VALUES ($1, $2, $3, $4)
...
pub async fn list_all_frames(
    State(state): State<AppState>,
    AdminClaims(_): AdminClaims,
) -> Result<Json<Vec<FrameRow>>, StatusCode> {
    let rows = sqlx::query_as::<_, FrameRow>(
        "SELECT id, name, price, currency, is_active, is_default FROM frames ORDER BY created_at DESC",
    )
    .fetch_all(&state.db)
    .await
...
pub async fn update_frame(
    ...
    AdminClaims(_): AdminClaims,
    Path(id): Path<Uuid>,
    Json(payload): Json<UpdateFrameRequest>,
) -> Result<Json<FrameRow>, StatusCode> {
